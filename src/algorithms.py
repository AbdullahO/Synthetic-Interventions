...
def hsvt_ols(
    ...
    """
    # set any observations in donor_units that is nan to zero in pre-intervention
    X1[np.isnan(X1)] = 0

    # any t with y1 = nan, remove from the matrix
    not_nan_indices = np.where(~np.isnan(y1))[0]
    X1_no_nan = X1[not_nan_indices, :]

    y1 = y1[not_nan_indices]
    # if there are no donors, then don't run method
    if X1_no_nan.shape[1] == 0:
        if include_pre:
    ...
    # ranks
    if method == "Donoho":
        k1 = approximate_rank2(X1_no_nan)
        k2 = approximate_rank2(X2)
    else:
        k1 = approximate_rank(X1_no_nan, t=t)
        k2 = approximate_rank(X2, t=t)
    # de-noise donor matrices
    X1_hat, u1, s1, v1 = hsvt(X1_no_nan, rank=k1, return_all=True)
    ...
    yh = (
        np.concatenate(
            [X1[(metric_i) * T0 : (metric_i + 1) * T0, :].dot(beta).T + c, y2]
        )
        if include_pre
        else y2
    )
    # estimate sigma
    sigma_1 = np.mean(np.square(X1_hat - X1_no_nan))
